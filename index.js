...
app.get('/samples/:type', async (req, res) => {
    ...
    if (typ === 'TM' || typ === 'CK' || typ === 'RE' || typ === 'ALL') {
        ...
        let query = "SELECT * FROM DEVELOPERS.minsal_integracion m where m.dtmdataaccettazione >= to_date('01/04/2021 00:00:00','dd/MM/yyyy hh24:mi:ss'"
...
const tmprocess = async () => {
    ...
    if (flag) {
        ...
        await database.simpleExecute(query, [], { autoCommit: true, maxRows: 30 }).then(async result => {
                ...
                for (const msg of result.rows) {
                    ...
                    await callMinsalJSON(newmsg.xmlMSG, '/crearMuestras').then(async result => {
                        ...
                        if (result.status !== 200) {
                            ...
                            //log(`Error CRUIZ => ` + JSON.stringify(result.data.error));
                        } else {
...
const ckprocess = async () => {
    ...
    if (flag) {
        oracledb.fetchAsString = [oracledb.CLOB]
        //let query = `SELECT * FROM DEVELOPERS.minsal_integracion WHERE STRTIPOMENSAJE = 'CK' AND BYTESTADO = 0 AND ZONA = 'ELSA' AND STRIDCAMPETICHETTA = '2892838602'`;
        ...
        await database.simpleExecute(query, [], { autoCommit: true, maxRows: 10 }).then(async result => {
            ...
            if (flag) {
                ...
                for (const msg of result.rows) {
                    ...
                    log("LLamando a WS entregaResultado.")
